	...
	for (int y = 0; y < h; y++) {
		for (int z = 0; z < d; z++) {
			for (int x = 0; x < w; x++, index++) {
				Chunk* chunk = new Chunk(x, y, z, true);
				chunks[index] = chunk;
...
Chunks::Chunks(int w, int h, int d, double** perlin) : w(w), h(h), d(d) {
...
voxel* Chunks::get(int x, int y, int z){
	int cx = x / CHUNKSIZE_W;
	int cy = y / CHUNKSIZE_H;
	int cz = z / CHUNKSIZE_D;
	if (x < 0) cx--;
	if (y < 0) cy--;
	if (z < 0) cz--;
	...
	int lx = x - cx * CHUNKSIZE_W;
	int ly = y - cy * CHUNKSIZE_H;
	int lz = z - cz * CHUNKSIZE_D;
	return &chunk->voxels[(ly * CHUNKSIZE_D + lz) * CHUNKSIZE_W + lx];
}
...
void Chunks::set(int x, int y, int z, int id){
	...
	int ly = y - cy * CHUNKSIZE_H;
	int lz = z - cz * CHUNKSIZE_D;
	chunk->voxels[(ly * CHUNKSIZE_D + lz) * CHUNKSIZE_W + lx].id = id;
	chunk->modified = true;
	...
	if (lx == CHUNKSIZE_W - 1 && (chunk = getChunk(cx + 1, cy, cz))) chunk->modified = true;
	if (ly == CHUNKSIZE_H - 1 && (chunk = getChunk(cx, cy + 1, cz))) chunk->modified = true;
	if (lz == CHUNKSIZE_D - 1 && (chunk = getChunk(cx, cy, cz + 1))) chunk->modified = true;
}
