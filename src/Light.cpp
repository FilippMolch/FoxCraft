...
Light::Light(Chunks* chunks, int channel) : chunks(chunks), channel(channel) {
	
}

void Light::add(int x, int y, int z) {
	add(x, y, z, chunks->getLight(x, y, z, channel));
}

void Light::add(int x, int y, int z, int emission) {
	if (emission <= 1)
		return;
	lightentry entry;
	entry.x = x;
...
void Light::remove(int x, int y, int z) {
